#!/bin/bash
#SBATCH -J dlv3p_vainf
#SBATCH -p gpu
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -c 4
#SBATCH --gres=gpu:1
#SBATCH -o runs/logs/deeplabv3plus_vainf_official_%j.out
#SBATCH -e runs/logs/deeplabv3plus_vainf_official_%j.out
...
echo "===== VAINF DEEPLABV3+ RESNET50 OS16 / STRICT 30K ====="
echo "Host: $(hostname)"
which python
python --version
nvidia-smi
...
python src/train_deeplabv3plus_vainf_official.py \
  --run-name deeplabv3plus_vainf_r50_os16_official_30k_bs4 \
  --image-size 512 \
  --batch-size 4 \
  --val-batch-size 4 \
  --total-iters 30000 \
...
cat runs/deeplabv3plus_vainf_r50_os16_official_30k_bs4/test_metrics.json
cat runs/deeplabv3plus_vainf_r50_os16_official_30k_bs4/summary_metrics.json
