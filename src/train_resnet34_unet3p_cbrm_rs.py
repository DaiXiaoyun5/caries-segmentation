...
class CariesBoundaryRefinementModuleRS(nn.Module):
    ...
    相比 CBRM：
    1. 保留 Local Boundary Branch；
    2. 保留 Dilated Boundary Branch；
    3. 保留 Gradient Cue Branch；
    4. 新增可学习残差缩放系数 gamma。

    设计目的：
    让模块只学习边界修正项，而不是强行重写原始 hd1 特征。
    初始 gamma=0，训练早期近似保持原始特征；
    后续由网络自动学习需要加入多少边界增强。
    """
...
# =========================
class ResNet34UNet3PlusCBRMRS(nn.Module):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_cbrm_rs")
