...
class UnetPPWithEdge(nn.Module):
    def __init__(self, encoder_name="resnet34", encoder_weights="imagenet", in_channels=3, classes=1):
        ...
        # segmentation_head[0] 通常是最后的 Conv2d
...
def save_prediction_panel(model, loader, device, out_path, max_items=4):
    ...
    for i in range(b):
        ...
        gt = masks[i, 0].numpy()
        edge = edges[i, 0].numpy()
        pd = seg_preds[i, 0].numpy()
...
# =========================
# 训练 / 验证 / 测试
# 当前阶段：只用 seg_logits 算主分割损失
# edge_logits 暂时只输出，不参与 loss
# =========================
def run_one_epoch(model, loader, optimizer, device, train=True):
    ...
    for images, masks, edges, _, _ in loader:
        images = images.to(device)
        masks = masks.to(device)
        edges = edges.to(device)  # 先读进来，后面加 edge loss 会用到
        ...
        with torch.set_grad_enabled(train):
            ...
            # 当前阶段：先不使用 edge_logits 参与 loss
            loss = bce_loss_fn(seg_logits, masks) + dice_loss_from_logits(seg_logits, masks)
...
def main(args):
    ...
    sample_images, sample_masks, sample_edges, _, _ = sample_batch
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_method_v1_edge_branch")
