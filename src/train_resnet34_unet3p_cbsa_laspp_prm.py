...
class PRMBlock(nn.Module):
    """
    PRM: Prediction Refinement Module.

    It refines the coarse segmentation logits using the final decoder feature hd1.
    This module is lightweight and works at prediction level:
        coarse_logits = segmentation_head(hd1)
        residual_logits = PRM(hd1, coarse_logits)
        final_logits = coarse_logits + scale * residual_logits

    Purpose:
    - CBSA handles local boundary-aware attention;
    - Lite-ASPP handles high-level multi-scale context;
    - PRM handles final prediction refinement, such as holes, burrs, and isolated false positives.
    """
    def __init__(
        self,
        feat_ch,
        ...
        self.refine = nn.Sequential(
            nn.Conv2d(feat_ch + 1, hidden_ch, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, feat, coarse_logits):
        ...
        residual_logits = self.refine(x)

        scale = torch.clamp(self.residual_scale, 0.0, 1.0)
        return coarse_logits + scale * residual_logits


class ResNet34UNet3PlusCBSALiteASPPPRM(ResNet34UNet3PlusCBSALiteASPP):
    """
    ResNet34-UNet3+ + CBSA + Lite-ASPP + PRM.

    Lite-ASPP is applied after encoder h5.
    CBSA is applied on decoder features hd4/hd3/hd2/hd1.
    PRM refines the final prediction logits from hd1.
    """
    def __init__(
        self,
        ...
        self.prm = PRMBlock(
            feat_ch=hd_ch,
            hidden_ch=prm_hidden,
            dropout=prm_dropout,
            residual_scale=prm_res_scale,
        )
    ...
    def forward(self, x):
        ...
        # Coarse prediction + PRM residual refinement.
        coarse_d1 = self.outconv1(hd1)
        refined_d1 = self.prm(hd1, coarse_d1)
        d1 = self.up_to_size(refined_d1, input_size)

        if not self.deep_supervision:
            return d1

        # Deep supervision keeps original auxiliary heads.
...
def main(args):
    ...
    model = ResNet34UNet3PlusCBSALiteASPPPRM(
        ...
        prm_hidden=args.prm_hidden,
        prm_dropout=args.prm_dropout,
        prm_res_scale=args.prm_res_scale,
    ).to(device)
    ...
    print("prm_hidden         =", args.prm_hidden)
    print("prm_dropout        =", args.prm_dropout)
    print("prm_res_scale      =", args.prm_res_scale)
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_cbsa_laspp_prm")
    ...
    parser.add_argument("--prm-hidden", type=int, default=64)
    parser.add_argument("--prm-dropout", type=float, default=0.0)
    parser.add_argument("--prm-res-scale", type=float, default=0.1)
