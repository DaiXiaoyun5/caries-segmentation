...
class ContrastDrivenFeatureEnhancement(nn.Module):
    """
    CDFE: Contrast-Driven Feature Enhancement.
    对比驱动特征增强模块。

    设计目的：
    1. 根据 decoder 高分辨率特征生成粗前景响应；
    2. 分别聚合前景上下文和背景上下文；
    3. 用前景-背景对比关系生成通道重标定权重；
    4. 增强龋齿区域与正常牙体组织之间的可分性。

    这个版本不引入额外监督，方便先做单模块验证。
    """
    def __init__(self, channels, reduction=4):
        super().__init__()
        hidden = max(channels // reduction, 32)

        self.coarse_mask = nn.Conv2d(channels, 1, kernel_size=1)
        ...
        self.local_refine = ConvBlock(channels, channels)

    def forward(self, x):
        # 粗前景概率，shape: [B,1,H,W]
        prob = torch.sigmoid(self.coarse_mask(x))

        # 前景/背景上下文，shape: [B,C,1,1]
        eps = 1e-6
        fg_context = (x * prob).sum(dim=(2, 3), keepdim=True) / (prob.sum(dim=(2, 3), keepdim=True) + eps)
        bg_prob = 1.0 - prob
        bg_context = (x * bg_prob).sum(dim=(2, 3), keepdim=True) / (bg_prob.sum(dim=(2, 3), keepdim=True) + eps)

        # 用前景和背景上下文生成通道级对比权重
        contrast = torch.cat([fg_context, bg_context], dim=1)
        gate = self.contrast_gate(contrast)

        # 对比增强 + 局部细化
        x = x * (1.0 + gate)
        return self.local_refine(x)


# =========================
# ResNet34 pretrained encoder + UNet3+ decoder
# =========================
class ResNet34UNet3PlusCDFE(nn.Module):
    """
    ...
    def __init__(
        ...
        # CDFE：对比驱动特征增强，作用在最高分辨率 decoder 特征 hd1 上
        self.cdfe = ContrastDrivenFeatureEnhancement(self.up_ch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_cdfe")
