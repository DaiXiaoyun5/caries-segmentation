...
class DCNv2Block(nn.Module):
    """
    基于 torchvision 的 DCNv2 (可变形卷积) 模块。
    让模型自动学习卷积核的偏移量，完美贴合极不规则的龋齿边界。
    """
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # 学习 offset (偏移量，2个方向) 和 mask (权重，1个标量)，3x3卷积核 = 3 * 9 = 27
        self.offset_mask_conv = nn.Conv2d(in_channels, 27, kernel_size=3, padding=1)
        # 初始时，让可变形卷积退化为普通卷积 (offset=0, mask=0.5，sigmoid(0)=0.5)
...
class ResNet34UNet3PlusEdgeDCN(nn.Module):
    ...
    def __init__(self, encoder_name="resnet34", encoder_weights="imagenet", in_channels=3, num_classes=1, cat_channels=64, deep_supervision=True):
        ...
        # 【核心缝合点 1】：hd2 特征较高分辨率，引入 DCN 捕捉细节边界
        ...
        # 【核心缝合点 2】：hd1 最终全分辨率特征融合，引入 DCN 贴合边缘
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resnet34_unet3p_edge_dcn_e50_bs6")
