...
class BoundaryRefinementModule(nn.Module):
    """
    BRM v2: Boundary Refinement Module.

    与 Edge+BRM v1 的区别：
    1. edge_head 使用原始 hd1 特征；
    2. BRM 只作用于 segmentation head；
    3. 避免 BRM 同时影响 edge 分支，减少边界监督和特征细化之间的冲突。
    """
...
# =========================
class ResNet34UNet3PlusEdgeBRMV2(nn.Module):
    def __init__(
        self,
    ...
    def forward(self, x):
        ...
        # v2 核心：edge 分支使用原始 hd1，保持边界监督不被 BRM 干扰
        edge_logits = self.up_to_size(self.edge_head(hd1), input_size)

        # BRM 只细化分割分支特征
        hd1_seg = self.brm(hd1)
        d1 = self.up_to_size(self.outconv1(hd1_seg), input_size)
...
def main(args):
    ...
    model = ResNet34UNet3PlusEdgeBRMV2(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_edge_brm_v2")
