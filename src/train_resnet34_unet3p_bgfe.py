...
class BoundaryDetection(nn.Module):
    """
    From PBE-UNet official implementation:
    decoder feature -> boundary logit.
    """
...
class BAFM(nn.Module):
    """
    Boundary-Aware Feature Modulation / BGFE core.

    This follows the official PBE-UNet idea:
    1. fuse main feature with boundary prediction
    2. propagate boundary into a multi-channel attention feature
    3. enhance feature by boundary-aware modulation + residual conv
    """
...
class ResNet34UNet3PlusBGFE(nn.Module):
    def __init__(
        self,
    ...
    def forward(self, x):
        ...
        edge_outputs = [
            self.up_to_size(b1, input_size),
            self.up_to_size(b2, input_size),
            self.up_to_size(b3, input_size),
            self.up_to_size(b4, input_size),
        ]

        if not self.deep_supervision:
            return d1, edge_outputs
...
def get_main_edge_logits(edge_outputs):
    return edge_outputs[0] if isinstance(edge_outputs, list) else edge_outputs
...
def compute_edge_loss(edge_outputs, edges, edge_loss_fn):
    if isinstance(edge_outputs, list):
        ...
        losses = [edge_loss_fn(edge_out, edges) for edge_out in edge_outputs]
        return sum(w * loss for w, loss in zip(weights, losses)) / sum(weights)
...
def save_prediction_panel(model, loader, device, out_path, edge_vis_threshold=0.2, max_items=4):
    ...
    with torch.no_grad():
        seg_outputs, edge_outputs = model(images)
        main_logits = get_main_logits(seg_outputs)
        edge_logits = get_main_edge_logits(edge_outputs)
...
def main(args):
    ...
    with torch.no_grad():
        seg_outputs, edge_outputs = model(sample_images)
        main_logits = get_main_logits(seg_outputs)
        main_edge_logits = get_main_edge_logits(edge_outputs)

    print("=" * 80)
    print("First forward check")
    print("input images.shape     =", sample_images.shape)
    print("main_logits.shape      =", main_logits.shape)
    print("main_edge_logits.shape =", main_edge_logits.shape)
    if isinstance(seg_outputs, list):
        print("seg deep outputs       =", [tuple(o.shape) for o in seg_outputs])
    if isinstance(edge_outputs, list):
        print("edge outputs           =", [tuple(o.shape) for o in edge_outputs])
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_bgfe")
