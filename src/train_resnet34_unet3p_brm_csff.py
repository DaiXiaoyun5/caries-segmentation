...
class BoundaryRefinementModule(nn.Module):
    ...
    设计目的：
    1. 强化高分辨率 decoder 特征中的局部边界响应；
    2. 改善边界模糊和轮廓不准；
    3. 本实验不使用 edge loss，用来和已有 Edge 分支做干净对比。
    """
...
class CrossStageFeatureFusion(nn.Module):
    ...
    设计目的：
    1. 使用 encoder 的 h1-h5 多阶段特征；
    2. 补充浅层纹理、中层结构和深层语义；
    3. 减少小病灶在下采样过程中的信息丢失。
    """
    def __init__(self, in_channels_list, cat_ch=64, out_ch=320):
        super().__init__()
        self.proj = nn.ModuleList([
            Conv1x1BNReLU(in_ch, cat_ch) for in_ch in in_channels_list
        ])

        # 学习不同 encoder stage 的重要性
        self.stage_logits = nn.Parameter(torch.zeros(len(in_channels_list)))

        # 5 个 stage 每个压到 cat_ch，拼接后再融合回 out_ch
        self.fuse = ConvBlock(cat_ch * len(in_channels_list), out_ch)

    def forward(self, features, ref_feature):
        weights = torch.softmax(self.stage_logits, dim=0)
        ref_size = ref_feature.shape[-2:]
        outs = []

        for feat, proj, w in zip(features, self.proj, weights):
            x = proj(feat)
...
class ResNet34UNet3PlusBRMCSFF(nn.Module):
    """
    ...
    def __init__(
        ...
        # CSFF：跨阶段特征融合，用 encoder h1-h5 补充小病灶信息
        self.csff = CrossStageFeatureFusion(
            in_channels_list=[ch1, ch2, ch3, ch4, ch5],
            cat_ch=cat_ch,
            out_ch=self.up_ch,
        )
        self.csff_fuse = ConvBlock(self.up_ch * 2, self.up_ch)
    ...
    def forward(self, x):
        ...
        csff_feat = self.csff([h1, h2, h3, h4, h5], h1)
        hd1 = self.csff_fuse(torch.cat([hd1, csff_feat], dim=1))
        hd1 = self.brm(hd1)
...
def main(args):
    ...
    model = ResNet34UNet3PlusBRMCSFF(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_brm_csff")
