...
class DepthwiseContextUnit(nn.Module):
    """
    Depthwise-separable 3x3 context unit used by DMRC-lite branches.
    """
...
class DMRCLiteBottleneck(nn.Module):
    """
    Dynamic Multi-Resolution Convolution lite bottleneck.

    Difference from SSGCB:
      - SSGCB: same-resolution branches with different dilation rates.
      - DMRC-lite: branches process the same feature at different resolutions.

    Branches:
      1) original resolution: DWConv 3x3
      2) 1/2 resolution: adaptive pool -> DWConv 3x3 -> upsample
      3) 1/4 resolution: adaptive pool -> DWConv 3x3 -> upsample
      4) global branch: GAP -> Conv1x1 -> upsample

    The branch weights are predicted from a global descriptor of the summed
    branch responses, then a 1x1 fusion is added back with bounded residual
    scale. Decoder and skip connections are untouched.
    """
    def __init__(
        self,
        channels,
        ...
        self.branch_full = DepthwiseContextUnit(mid_channels)
        self.branch_half = DepthwiseContextUnit(mid_channels)
        self.branch_quarter = DepthwiseContextUnit(mid_channels)
        self.branch_global = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(mid_channels, mid_channels, kernel_size=1, bias=True),
            nn.SiLU(inplace=True),
        )

        mlp_hidden = max(mid_channels // 4, 16)
        self.branch_mlp = nn.Sequential(
            nn.Linear(mid_channels, mlp_hidden, bias=True),
            nn.ReLU(inplace=True),
            nn.Linear(mlp_hidden, 4, bias=True),
        )
    ...
    @staticmethod
    def _pooled_branch(x, branch, scale):
        h, w = x.shape[-2:]
        pooled_h = max(1, h // int(scale))
        pooled_w = max(1, w // int(scale))
        pooled = F.adaptive_avg_pool2d(x, output_size=(pooled_h, pooled_w))
        out = branch(pooled)
    ...
    def forward(self, x):
        ...
        b_full = self.branch_full(x_mid)
        b_half = self._pooled_branch(x_mid, self.branch_half, scale=2)
        b_quarter = self._pooled_branch(x_mid, self.branch_quarter, scale=4)
        b_global = self.branch_global(x_mid)
        b_global = F.interpolate(b_global, size=size, mode="bilinear", align_corners=False)

        branches = [b_full, b_half, b_quarter, b_global]
        u = b_full + b_half + b_quarter + b_global
        z = F.adaptive_avg_pool2d(u, 1).flatten(1)
        weights = torch.softmax(self.branch_mlp(z), dim=1)
...
class ResNet34UNetBRRB34DMRC(nn.Module):
    """
    B34: B2 + DMRC-lite bottleneck context.

    Keeps:
      - original B2 boundary/rim auxiliary supervision
      - original B2 final RBSG-lite
      - decoder and skip connections unchanged

    Adds:
      - one dynamic multi-resolution context block on the deepest encoder
        bottleneck feature before the U-Net decoder.
    """
    def __init__(
        self,
        ...
        self.dmrc = DMRCLiteBottleneck(
            channels=bottleneck_ch,
            mid_channels=dmrc_mid_channels,
            dropout=dmrc_dropout,
            scale_init=dmrc_scale_init,
            scale_max=dmrc_scale_max,
        )
    ...
    def forward(self, x, return_aux=False):
        ...
        features[-1] = self.dmrc(features[-1])
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetBRRB34DMRC"
    config["experiment"] = "B34: B2 BRR-RBSG + DMRC-lite bottleneck"
    config["note"] = (
        "B34 keeps the original B2 boundary/rim auxiliary supervision and final "
        "RBSG-lite. It adds a DMC-Net-inspired DMRC-lite module on the deepest "
        "encoder bottleneck feature: original, 1/2, 1/4 and global resolution "
        "branches with dynamic branch fusion and bounded residual output. Decoder "
        "and skip connections are unchanged."
    )
    config["aux_targets"] = {
    ...
    config["dmrc"] = {
        ...
        "mid_channels": args.dmrc_mid_channels,
        "branches": ["original", "1/2 pooled", "1/4 pooled", "global pooling"],
        "branch_op": "depthwise-separable 3x3 for spatial branches",
        "dynamic_fusion": "softmax(MLP(GAP(sum(branches))))",
        "dropout": args.dmrc_dropout,
        "scale_init": args.dmrc_scale_init,
        "scale_max": args.dmrc_scale_max,
    ...
    model = ResNet34UNetBRRB34DMRC(
        encoder_name=args.encoder_name,
        encoder_weights=args.encoder_weights,
        in_channels=3,
        num_classes=1,
        dmrc_mid_channels=args.dmrc_mid_channels,
        dmrc_dropout=args.dmrc_dropout,
        dmrc_scale_init=args.dmrc_scale_init,
        dmrc_scale_max=args.dmrc_scale_max,
    ).to(device)

    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    print("=" * 80)
    print("Model ready: B34 DMRC-lite + B2 BRR-RBSG ResNet34-U-Net")
    ...
    print("lr              =", args.lr)
    print("dmrc_mid_channels =", args.dmrc_mid_channels)
    print("dmrc_scale_init   =", args.dmrc_scale_init)
    print("dmrc_scale_max    =", args.dmrc_scale_max)
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="b34_dmrc_auglite_e260_constlr_bs6")
    ...
    parser.add_argument("--dmrc-mid-channels", type=int, default=96)
    parser.add_argument("--dmrc-dropout", type=float, default=0.0)
    parser.add_argument("--dmrc-scale-init", type=float, default=0.05)
    parser.add_argument("--dmrc-scale-max", type=float, default=0.30)
