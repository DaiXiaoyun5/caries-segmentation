...
class ResNet34UNetDS1CBSAMKDC(nn.Module):
    """
    DS1:
    ResNet34-U-Net + CBSA + MKDC with deep supervision.

    Main prediction:
        final output after CBSA + MKDC: [B, 1, 512, 512]

    Auxiliary predictions:
        aux_128 from decoder.blocks[2]: [B, 1, 128, 128] -> upsample to target size for loss
        aux_256 from decoder.blocks[3]: [B, 1, 256, 256] -> upsample to target size for loss

    Inference/evaluation uses only the final output.
    """
    def __init__(
        self,
        ...
        # According to inspected SMP 0.3.3 decoder:
        # decoder.blocks[2] -> [B, 64, 128, 128]
        # decoder.blocks[3] -> [B, 32, 256, 256]
        self.aux_head_128 = nn.Conv2d(64, num_classes, kernel_size=1)
        self.aux_head_256 = nn.Conv2d(32, num_classes, kernel_size=1)
    ...
    def _decode_with_aux(self, features):
        # Reproduce SMP 0.3.3 UnetDecoder.forward to expose intermediate features.
        decoder_features = features[1:]
        decoder_features = decoder_features[::-1]

        head = decoder_features[0]
        skips = decoder_features[1:]
        ...
        for i, decoder_block in enumerate(self.unet.decoder.blocks):
            ...
            if i == 2:
                aux_128 = self.aux_head_128(x)
            elif i == 3:
                aux_256 = self.aux_head_256(x)

        return x, aux_128, aux_256

    def forward_with_aux(self, x):
        features = self.unet.encoder(x)
        decoder_output, aux_128, aux_256 = self._decode_with_aux(features)

        decoder_output = self.cbsa(decoder_output)
        decoder_output = self.mkdc_refine(decoder_output)
        final_logits = self.unet.segmentation_head(decoder_output)

        return final_logits, aux_256, aux_128

    def forward(self, x):
        final_logits, _, _ = self.forward_with_aux(x)
        return final_logits


def bce_dice_loss_from_logits(logits, targets, eps=1e-7):
    bce = F.binary_cross_entropy_with_logits(logits, targets)
    probs = torch.sigmoid(logits)
    dims = (1, 2, 3)
    intersection = torch.sum(probs * targets, dim=dims)
    cardinality = torch.sum(probs + targets, dim=dims)
    dice_loss = 1.0 - torch.mean((2.0 * intersection + eps) / (cardinality + eps))
    return bce + dice_loss


def deep_supervision_loss(final_logits, aux_256, aux_128, targets, aux256_weight=0.5, aux128_weight=0.25):
    final_loss = bce_dice_loss_from_logits(final_logits, targets)

    aux_256_up = F.interpolate(aux_256, size=targets.shape[-2:], mode="bilinear", align_corners=False)
    aux_128_up = F.interpolate(aux_128, size=targets.shape[-2:], mode="bilinear", align_corners=False)

    aux256_loss = bce_dice_loss_from_logits(aux_256_up, targets)
    aux128_loss = bce_dice_loss_from_logits(aux_128_up, targets)

    total_loss = final_loss + aux256_weight * aux256_loss + aux128_weight * aux128_loss

    return total_loss, {
        "final_loss": float(final_loss.detach().cpu().item()),
        "aux256_loss": float(aux256_loss.detach().cpu().item()),
        "aux128_loss": float(aux128_loss.detach().cpu().item()),
        "total_loss": float(total_loss.detach().cpu().item()),
    }


def compute_batch_metrics_from_logits(logits, targets, threshold=0.5, eps=1e-7):
    probs = torch.sigmoid(logits)
    ...
    dice = torch.mean((2.0 * tp + eps) / (2.0 * tp + fp + fn + eps))
    iou = torch.mean((tp + eps) / (tp + fp + fn + eps))
    precision = torch.mean((tp + eps) / (tp + fp + eps))
    recall = torch.mean((tp + eps) / (tp + fn + eps))
...
def run_one_epoch_ds(model, loader, optimizer, device, train=True, aux256_weight=0.5, aux128_weight=0.25):
    if train:
        model.train()
    else:
        model.eval()
    ...
    total_aux256_loss = 0.0
    total_aux128_loss = 0.0
    total_dice = 0.0
    total_iou = 0.0
    total_precision = 0.0
    total_recall = 0.0
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(train):
            final_logits, aux_256, aux_128 = model.forward_with_aux(images)
            loss, loss_parts = deep_supervision_loss(
                final_logits,
                aux_256,
                aux_128,
                masks,
                aux256_weight=aux256_weight,
                aux128_weight=aux128_weight,
            )
        ...
        metrics = compute_batch_metrics_from_logits(final_logits.detach(), masks.detach())

        total_loss += float(loss.detach().cpu().item())
        total_final_loss += loss_parts["final_loss"]
        total_aux256_loss += loss_parts["aux256_loss"]
        total_aux128_loss += loss_parts["aux128_loss"]
    ...
    return {
        "loss": total_loss / n,
        "final_loss": total_final_loss / n,
        "aux256_loss": total_aux256_loss / n,
        "aux128_loss": total_aux128_loss / n,
        "dice": total_dice / n,
        "iou": total_iou / n,
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetDS1CBSAMKDC"
    config["experiment"] = "DS1: ResNet34-U-Net + CBSA + MKDC with deep supervision"
    config["note"] = "Architecture is based on P2. Deep supervision is added at decoder.blocks[2] and decoder.blocks[3] with auxiliary heads at 128x128 and 256x256. Inference uses only the final output."
    ...
    print("=" * 80)
    print("Model ready: DS1 ResNet34-U-Net + CBSA + MKDC with deep supervision")
    ...
    print("aux256_weight        =", args.aux256_weight)
    print("aux128_weight        =", args.aux128_weight)
    ...
    with open(history_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            "epoch",
            "train_loss", "train_final_loss", "train_aux256_loss", "train_aux128_loss",
            "train_dice", "train_iou", "train_precision", "train_recall",
            "val_loss", "val_final_loss", "val_aux256_loss", "val_aux128_loss",
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = run_one_epoch_ds(model, train_loader, optimizer, device, train=True, aux256_weight=args.aux256_weight, aux128_weight=args.aux128_weight)
        val_metrics = run_one_epoch_ds(model, val_loader, optimizer=None, device=device, train=False, aux256_weight=args.aux256_weight, aux128_weight=args.aux128_weight)

        print(
            f"[Epoch {epoch:03d}/{args.epochs}] "
            f"train_loss={train_metrics['loss']:.4f} "
            f"train_final={train_metrics['final_loss']:.4f} "
            f"train_aux256={train_metrics['aux256_loss']:.4f} "
            f"train_aux128={train_metrics['aux128_loss']:.4f} "
            f"train_dice={train_metrics['dice']:.4f} "
            f"val_loss={val_metrics['loss']:.4f} "
            f"val_final={val_metrics['final_loss']:.4f} "
        ...
        with open(history_path, "a", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                epoch,
                train_metrics["loss"], train_metrics["final_loss"], train_metrics["aux256_loss"], train_metrics["aux128_loss"],
                train_metrics["dice"], train_metrics["iou"], train_metrics["precision"], train_metrics["recall"],
                val_metrics["loss"], val_metrics["final_loss"], val_metrics["aux256_loss"], val_metrics["aux128_loss"],
    ...
    best_train_metrics = run_one_epoch_ds(model, train_loader, optimizer=None, device=device, train=False, aux256_weight=args.aux256_weight, aux128_weight=args.aux128_weight)
    best_val_metrics = run_one_epoch_ds(model, val_loader, optimizer=None, device=device, train=False, aux256_weight=args.aux256_weight, aux128_weight=args.aux128_weight)
    test_metrics = run_one_epoch_ds(model, test_loader, optimizer=None, device=device, train=False, aux256_weight=args.aux256_weight, aux128_weight=args.aux128_weight)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_cbsa_mkdc_ds1_e50_bs6")
    ...
    parser.add_argument("--aux256-weight", type=float, default=0.5)
    parser.add_argument("--aux128-weight", type=float, default=0.25)
