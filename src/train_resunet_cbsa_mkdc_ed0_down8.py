...
class BottleneckDown8Context(nn.Module):
    """
    Extra bottleneck downsampling context module.

    For P2 with 512 input:
        f5: [B, 512, 16, 16]

    It further downsamples the bottleneck feature:
        16x16 -> 8x8
    then refines and upsamples it back to 16x16.
    Context is injected with a residual scaling coefficient beta.
    """
    def __init__(self, channels=512, beta=0.1, activation="relu"):
        super().__init__()
        if activation == "relu":
...
class ResNet34UNetDown8CBSAMKDC(nn.Module):
    """
    ED0-D8:
    ResNet34-U-Net + extra 16x16->8x8 bottleneck context + CBSA + MKDC.

    Only modification against P2:
    - Insert a residual bottleneck context module after features[-1].
    - It downsamples the 16x16 feature to 8x8 and upsamples it back before decoder.
    """
    def __init__(
        self,
        ...
        self.bottleneck_down8 = BottleneckDown8Context(
            channels=512,
            beta=down8_beta,
            activation="relu",
        )
    ...
    def forward(self, x):
        ...
        features[-1] = self.bottleneck_down8(features[-1])
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetDown8CBSAMKDC"
    config["experiment"] = "ED0-D8: ResNet34-U-Net + bottleneck downsample-to-8 context + CBSA + MKDC"
    config["note"] = "Architecture is based on P2. A residual bottleneck context module is inserted after the deepest ResNet34 encoder feature. It downsamples 16x16 to 8x8 and upsamples back before the original decoder. Input processing, loss, CBSA and MKDC remain unchanged."
    ...
    model = ResNet34UNetDown8CBSAMKDC(
    ...
    print("=" * 80)
    print("Model ready: ED0-D8: ResNet34-U-Net + bottleneck downsample-to-8 context + CBSA + MKDC")
    ...
    print("down8_beta           =", args.down8_beta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_cbsa_mkdc_ed0_down8_e50_bs6")
    ...
    parser.add_argument("--down8-beta", type=float, default=0.1)
