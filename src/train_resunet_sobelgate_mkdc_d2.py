...
class SobelEdgeGateBlock(nn.Module):
    """
    Sobel Edge Gate block.

    This is an edge-guided ablation against CBSA:
    - SimAM attention is removed.
    - Sobel gradient extraction is applied to decoder features.
    - Edge response is normalized to [0, 1].
    - A residual edge gate enhances decoder features conservatively.
    """
    def __init__(self, alpha=1.0, beta=0.1):
        ...
        # register_buffer means these fixed Sobel kernels move with model.to(device),
    ...
    def sobel_gradient(self, x):
        ...
        # Depthwise Sobel: each channel is filtered independently.
        weight_x = self.sobel_x.repeat(c, 1, 1, 1)
        weight_y = self.sobel_y.repeat(c, 1, 1, 1)

        gx = F.conv2d(x, weight_x, bias=None, stride=1, padding=1, groups=c)
        gy = F.conv2d(x, weight_y, bias=None, stride=1, padding=1, groups=c)

        edge = torch.abs(gx) + torch.abs(gy)

        # Collapse channel responses to a single spatial edge cue.
    ...
    def forward(self, x):
        edge = self.sobel_gradient(x)
        gate = 1.0 + self.alpha * edge
...
class ResNet34UNetSobelGateMKDC(nn.Module):
    """
    D2: ResNet34-U-Net + Sobel Edge Gate + MKDC/MKIR

    Fairness design:
    - SMP ResNet34-U-Net main body unchanged.
    - Original CBSA is replaced by Sobel Edge Gate.
    - MKDC/MKIR block remains inserted as decoder-output refinement before segmentation head.
    - This experiment isolates whether Sobel-based edge gating is more useful than CBSA/D1 boundary cues.
    """
    def __init__(
        self,
        ...
        self.sobel_edge_gate = SobelEdgeGateBlock(
            alpha=cbsa_alpha,
            beta=cbsa_beta,
        )
    ...
    def forward(self, x):
        ...
        decoder_output = self.sobel_edge_gate(decoder_output)
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetSobelGateMKDC"
    config["experiment"] = "D2: ResNet34-U-Net + Sobel Edge Gate + MKDC/MKIR"
    config["note"] = "CBSA is replaced by Sobel Edge Gate: Sobel gradient extraction on decoder features -> edge normalization -> edge gate -> residual feature enhancement. MKDC remains decoder-output refinement."
    ...
    model = ResNet34UNetSobelGateMKDC(
    ...
    print("=" * 80)
    print("Model ready: D2 ResNet34-U-Net + Sobel Edge Gate + MKDC")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_sobelgate_mkdc_d2_e50_bs6")
