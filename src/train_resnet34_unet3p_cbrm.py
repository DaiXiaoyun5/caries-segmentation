...
class CariesBoundaryRefinementModule(nn.Module):
    """
    CBRM: Caries Boundary Refinement Module.
    龋齿边界细化模块。

    相比基础 BRM，本模块增加两个任务定制分支：
    1. Dilated Boundary Branch:
       使用 dilation=2 的 3x3 卷积扩大边界上下文感受野，
       用于捕捉 CariXray 中模糊、低对比龋齿边界周围信息。

    2. Gradient Cue Branch:
       使用固定 Sobel 算子从高分辨率 decoder 特征中提取梯度提示，
       用于增强弱边界和灰度变化区域响应。

    融合方式：
       F_refine = F + A_b * (F_local + F_dilated + F_grad)

    其中 A_b 是由三类边界上下文共同生成的空间边界注意力图。
    """
    def __init__(self, channels, reduction=4):
        ...
        # 边界注意力：由 local + dilated + gradient 三类上下文共同生成
    ...
    def forward(self, x):
        ...
        refined = x + context * boundary_att
...
# =========================
class ResNet34UNet3PlusCBRM(nn.Module):
    """
    ...
    def __init__(
        ...
        # CBRM：龋齿边界细化模块，作用在最高分辨率 decoder 特征 hd1 上
        self.cbrm = CariesBoundaryRefinementModule(self.up_ch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_cbrm")
