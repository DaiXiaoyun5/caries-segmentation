...
class CBSABlock(nn.Module):
    """
    CBSA: Caries Boundary-aware CBSA Attention.

    It keeps the parameter-free CBSA energy attention, then adds a lightweight
    feature-gradient boundary cue and residual scaling. This is designed for
    weak-boundary caries segmentation rather than generic attention stacking.
    """
    def __init__(self, e_lambda=1e-4, alpha=0.2, beta=0.1):
        super().__init__()
    ...
    def _feature_boundary_cue(self, x):
        # Use feature-gradient magnitude as a weak boundary cue.
        # It is computed from decoder features, so no extra edge label is needed for CBSA itself.
        ...
        g_std = g.std(dim=(2, 3), keepdim=True).clamp_min(1e-6)
        g = (g - g_mean) / g_std

        return torch.sigmoid(g)

    def forward(self, x):
        cbsa_att = self._cbsa_attention(x)
        boundary_cue = self._feature_boundary_cue(x)

        alpha = torch.clamp(self.alpha, 0.0, 1.0)
        beta = torch.clamp(self.beta, 0.0, 1.0)

        fused_att = cbsa_att * (1.0 + alpha * boundary_cue)
        return x + beta * x * fused_att

class ResNet34UNet3PlusEdgeCBSA(ResNet34UNet3PlusEdge):
    """
    ResNet34-UNet3+ + Edge Branch + CBSA.
    CBSA is inserted after decoder full-scale fusion features.
    """
    def __init__(
        self,
        ...
        self.cbsa_position = cbsa_position

        self.cbsa_hd4 = CBSABlock(e_lambda=cbsa_lambda, alpha=cbsa_alpha, beta=cbsa_beta)
        self.cbsa_hd3 = CBSABlock(e_lambda=cbsa_lambda, alpha=cbsa_alpha, beta=cbsa_beta)
        self.cbsa_hd2 = CBSABlock(e_lambda=cbsa_lambda, alpha=cbsa_alpha, beta=cbsa_beta)
        self.cbsa_hd1 = CBSABlock(e_lambda=cbsa_lambda, alpha=cbsa_alpha, beta=cbsa_beta)

    def maybe_cbsa(self, name, x):
        if self.cbsa_position == "none":
            return x

        if self.cbsa_position == "all":
            return getattr(self, f"cbsa_{name}")(x)

        if self.cbsa_position == name:
            return getattr(self, f"cbsa_{name}")(x)
...
def main(args):
    ...
    model = ResNet34UNet3PlusEdgeCBSA(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_edge_cbsa")
