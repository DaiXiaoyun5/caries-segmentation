...
class LiteASPPBlock(nn.Module):
    ...
    Purpose:
    - supplement multi-scale semantic context;
    - avoid overlapping with CBSA's local boundary-aware attention;
    - keep memory cost low because h5 is only 1/32 resolution.
    """
    def __init__(
        self,
        in_ch,
...
class ResNet34UNet3PlusCBSALiteASPP(ResNet34UNet3PlusCBSA):
    """
    ResNet34-UNet3+ + CBSA + Lite-ASPP.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_cbsa_laspp")
