...
class ResNet34UNetCBSA(nn.Module):
    """
    M3/AUG-CBSA:
    ResNet34-U-Net + final CBSA, without MKDC.

    This model is used to test whether MKDC is necessary under the
    effective AugLite training strategy.
    """
    def __init__(
        self,
        ...
        cbsa_alpha=1.0,
        cbsa_beta=0.1,
        # Keep these ignored arguments for compatibility with old configs/eval scripts.
...
def main(args):
    ...
    config["experiment"] = "AUG-M3-E200-CosLR: ResNet34-U-Net + CBSA without MKDC, AugLite, 200 epochs, cosine LR decay"
    config["note"] = "Based on M3/CBSA-only. MKDC is removed. AugLite is used for training only. Training is extended to 200 epochs with cosine learning-rate decay from 3e-4 to 1e-6."
    ...
    print("=" * 80)
    print("Model ready: ResNet34-U-Net + CBSA + AugLite, without MKDC")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_cbsa_auglite_e200_coslr_bs6")
