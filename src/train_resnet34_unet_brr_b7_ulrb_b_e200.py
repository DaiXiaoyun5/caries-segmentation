...
class ResNet34UNetBRRB7ULRBB(nn.Module):
    """
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetBRRB7ULRBB"
    config["experiment"] = "B7B: B2 BRR-RBSG + ULRB-gated"
    config["note"] = "B7B keeps original B2 RBSG-lite and adds an uncertainty-guided lesion recovery branch. Compared with B7A, B7B uses a soft uncertainty gate: final_logits = mask_logits + alpha * (0.5 + 0.5U) * delta_logits."
    config["aux_targets"] = {
    ...
    print("=" * 80)
    print("Model ready: B7B: B2 BRR-RBSG + ULRB-gated")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="b7b_b2_ulrb_gated_auglite_e200_constlr_bs6")
