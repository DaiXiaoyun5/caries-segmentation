...
class ResNet34UNetDSA1CBSAMKDC(nn.Module):
    """
    DSA1: ResNet34-U-Net + late decoder CBSA-lite + final CBSA + MKDC.

    Motivation:
    - Inspired by multi-level decoder attention/boundary-guided refinement.
    - Do not modify encoder or skip features directly.
    - Insert weak CBSA-lite after late decoder blocks:
        decoder.blocks[2] output: [B, 64, 128, 128]
        decoder.blocks[3] output: [B, 32, 256, 256]
    - Keep the original final decoder-output CBSA before MKDC:
        decoder.blocks[4] output: [B, 16, 512, 512]
    """
    def __init__(
        self,
        ...
        cbsa_alpha=1.0,
        cbsa_beta=0.1,
        stage_cbsa_alpha=1.0,
        stage_cbsa_beta=0.05,
        ...
        # According to inspection under SMP 0.3.3:
        # decoder.blocks[2] -> [B, 64, 128, 128]
        # decoder.blocks[3] -> [B, 32, 256, 256]
        # decoder.blocks[4] -> [B, 16, 512, 512]
        self.stage_cbsa_128 = CBSABlock(
            e_lambda=cbsa_lambda,
            alpha=stage_cbsa_alpha,
            beta=stage_cbsa_beta,
        )
        self.stage_cbsa_256 = CBSABlock(
            e_lambda=cbsa_lambda,
            alpha=stage_cbsa_alpha,
            beta=stage_cbsa_beta,
        )
    ...
    def forward(self, x):
        ...
        # Manually reproduce SMP 0.3.3 UnetDecoder.forward behavior:
        # remove the first same-resolution feature, reverse order, then pass head + skips.
        ...
        decoder_output = self.unet.decoder.center(head)

        for i, decoder_block in enumerate(self.unet.decoder.blocks):
            skip = skips[i] if i < len(skips) else None
            decoder_output = decoder_block(decoder_output, skip)

            # Late decoder CBSA-lite:
            # blocks[2]: [B, 64, 128, 128]
            # blocks[3]: [B, 32, 256, 256]
            if i == 2:
                decoder_output = self.stage_cbsa_128(decoder_output)
            elif i == 3:
                decoder_output = self.stage_cbsa_256(decoder_output)

        # Original final P2 refinement:
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetDSA1CBSAMKDC"
    config["experiment"] = "DSA1: ResNet34-U-Net + late decoder CBSA-lite + final CBSA + MKDC"
    config["note"] = "Inspired by multi-level decoder attention. Weak CBSA-lite is inserted after decoder.blocks[2] and decoder.blocks[3]; original final CBSA and MKDC are kept unchanged."
    ...
    model = ResNet34UNetDSA1CBSAMKDC(
        ...
        stage_cbsa_alpha=args.stage_cbsa_alpha,
        stage_cbsa_beta=args.stage_cbsa_beta,
    ...
    print("=" * 80)
    print("Model ready: DSA1 ResNet34-U-Net + late decoder CBSA-lite + final CBSA + MKDC")
    ...
    print("stage_cbsa_alpha     =", args.stage_cbsa_alpha)
    print("stage_cbsa_beta      =", args.stage_cbsa_beta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_dsa1_cbsa_mkdc_e50_bs6")
    ...
    parser.add_argument("--stage-cbsa-alpha", type=float, default=1.0)
    parser.add_argument("--stage-cbsa-beta", type=float, default=0.05)
