...
def estimate_edge_pos_weight(split_file, target_size=(512, 512), edge_kernel_size=3, max_samples=800):
    ...
    for line in chosen:
        ...
        pos_pixels += edge_np.sum()
        total_pixels += edge_np.size

    neg_pixels = total_pixels - pos_pixels
    pos_ratio = float(pos_pixels / max(total_pixels, 1))
    pos_weight = float(neg_pixels / max(pos_pixels, 1.0))

    pos_weight = max(1.0, min(pos_weight, 50.0))

    return {
        "samples_used": len(chosen),
        "pos_pixels": float(pos_pixels),
...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels=None, rates=(1, 6, 12, 18)):
        super().__init__()
        if out_channels is None:
            out_channels = in_channels

        inter_channels = max(in_channels // 4, 64)
        ...
        self.project = nn.Sequential(
            nn.Conv2d(len(rates) * inter_channels, out_channels, kernel_size=1, bias=False),
...
# =========================
# 模型：U-Net++ + ASPP + Edge Branch
# =========================
class UnetPPWithEdgeASPP(nn.Module):
    def __init__(self, encoder_name="resnet34", encoder_weights="imagenet", in_channels=3, classes=1):
        ...
        self.aspp = ASPP(in_channels=deepest_channels, out_channels=deepest_channels)
...
# 保持和 v1_edge_fix 一样：
# seg_loss = BCE + Dice
# edge_loss = Weighted BCE
# =========================
def run_one_epoch(model, loader, optimizer, device, lambda_edge=0.3, edge_pos_weight=1.0, train=True):
    if train:
        model.train()
    else:
        model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_method_v1_edge_aspp")
