...
class CariesBoundaryRefinementModuleRS(nn.Module):
    """
    CBRM-RS: Caries Boundary Refinement Module with Residual Scaling.
    带残差缩放的龋齿边界细化模块。

    组成：
    1. Local Boundary Branch：局部边界上下文；
    2. Dilated Boundary Branch：膨胀边界上下文；
    3. Gradient Cue Branch：Sobel 梯度提示；
    4. Residual Scaling：可学习残差缩放 gamma。

    作用：
    针对 CariXray 龋齿区域弱边界、低对比和背景误分问题，
    在 hd1 高分辨率解码特征上学习边界修正项。
    """
    def __init__(self, channels, reduction=4):
        ...
        self.gradient_project = nn.Sequential(
        ...
        self.update_refine = ConvBlock(channels, channels)

        # gamma 初始为 0，训练早期尽量不破坏原始 hd1 特征
    ...
    def forward(self, x):
        ...
        grad_feat = torch.sqrt(grad_x * grad_x + grad_y * grad_y + 1e-6)
        grad_feat = self.gradient_project(grad_feat)

        att_input = torch.cat([local_feat, dilated_feat, grad_feat], dim=1)
        boundary_att = self.boundary_attention(att_input)

        context = local_feat + dilated_feat + grad_feat
        update = self.update_refine(context * boundary_att)

        return x + self.gamma * update


class CariesSizeAwareDecoder(nn.Module):
    """
    CSAD: Caries Size-Aware Decoder.
    龋齿尺度感知解码模块。

    设计动机：
    CariXray 中龋齿病灶面积差异明显，小病灶需要局部细节，
    中/大病灶需要更大上下文。该模块用不同膨胀率的并行分支
    建模不同尺度龋齿区域，再自适应融合。

    分支含义：
    dilation=1：偏小病灶和局部细节；
    dilation=2：偏中尺度病灶；
    dilation=3：偏大尺度上下文。
    """
    def __init__(self, channels, reduction=4):
        ...
        # 残差缩放，初始为 0，避免尺度分支一开始破坏原始特征
        self.gamma = nn.Parameter(torch.zeros(1))

    def forward(self, x):
        small = self.small_branch(x)
        medium = self.medium_branch(x)
        large = self.large_branch(x)

        # [B, 3, 1, 1]
        weights = torch.softmax(self.scale_gate(x), dim=1)
...
# =========================
class ResNet34UNet3PlusCBRMRSCSAD(nn.Module):
    """
    ...
    def __init__(
        ...
        # CBRM-RS：龋齿边界细化模块
        self.cbrm_rs = CariesBoundaryRefinementModuleRS(self.up_ch)

        # CSAD：龋齿尺度感知解码模块
        self.csad = CariesSizeAwareDecoder(self.up_ch)
...
def main(args):
    ...
    model = ResNet34UNet3PlusCBRMRSCSAD(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_cbrm_rs_sad")
