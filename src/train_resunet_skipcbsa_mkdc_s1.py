...
class ResNet34UNetSkipCBSAMKDC(nn.Module):
    """
    S1:
    ResNet34-U-Net + Skip-CBSA + MKDC/MKIR refinement.

    Fairness design:
    - SMP ResNet34-U-Net main body unchanged.
    - CBSA is moved from decoder output to encoder skip features.
    - CBSA is applied only to features[1], features[2], features[3].
    - MKDC/MKIR remains a decoder-output refinement block before segmentation head.
    - Training setting should match P2: 512 input, batch size 6, 50 epochs, BCE+Dice.
    """
    def __init__(
        self,
        ...
        self.skip_indices = tuple(int(i) for i in skip_indices)
        for idx in self.skip_indices:
            if idx <= 0 or idx >= n_feats:
                raise ValueError(
                    f"Invalid skip index {idx}. For SMP ResNet34 encoder, "
                    f"valid skip feature indices are usually 1..{n_feats - 2}; got channels={encoder_channels}."
                )

        # CBSA is channel-agnostic, but separate module names make the architecture explicit.
        self.skip_cbsa_blocks = nn.ModuleDict({
            str(idx): CBSABlock(
    ...
    def forward(self, x):
        ...
        # Apply CBSA to selected skip features before they enter the U-Net decoder.
        # features[1], features[2], features[3] are shallow/mid-level features
        # with richer boundary and texture information.
        for idx in self.skip_indices:
            features[idx] = self.skip_cbsa_blocks[str(idx)](features[idx])

        decoder_output = self.unet.decoder(*features)

        # MKDC remains after decoder fusion, before segmentation head.
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetSkipCBSAMKDC"
    config["experiment"] = "S1: ResNet34-U-Net + Skip-CBSA(features[1,2,3]) + MKDC/MKIR"
    config["note"] = "CBSA moved from decoder output to skip features[1,2,3]; MKDC/MKIR remains decoder-output refinement. This is a position ablation against P2."
    ...
    model = ResNet34UNetSkipCBSAMKDC(
    ...
    print("=" * 80)
    print("Model ready: S1 ResNet34-U-Net + Skip-CBSA + MKDC")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_skipcbsa_mkdc_s1_e50_bs6")
