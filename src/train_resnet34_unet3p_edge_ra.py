...
class ReverseAttentionRefine(nn.Module):
    """
    Reverse Attention Refinement.

    coarse_logits gives a coarse lesion prediction.
    reverse attention = 1 - sigmoid(coarse_logits)
    The model uses reverse-attended features to predict residual logits.
    ...
    def forward(self, feature, coarse_logits):
        reverse_attention = 1.0 - torch.sigmoid(coarse_logits)
        refined_feature = feature * reverse_attention
        residual_logits = self.refine(refined_feature)

        final_logits = coarse_logits + self.scale * residual_logits

        return final_logits, residual_logits, reverse_attention


class ResNet34UNet3PlusEdgeRA(ResNet34UNet3PlusEdge):
    """
    ResNet34-UNet3+ + Edge Branch + Reverse Attention Refinement.

    RA is inserted on hd1, the highest-resolution decoder feature.
    """
    def __init__(
        self,
        ...
        self.ra_refine = ReverseAttentionRefine(self.up_ch)
    ...
    def forward(self, x):
        ...
        coarse_logits_hd1 = self.outconv1(hd1)
        refined_logits_hd1, residual_logits_hd1, reverse_attention = self.ra_refine(
            hd1,
            coarse_logits_hd1,
        )

        d1 = self.up_to_size(refined_logits_hd1, input_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_edge_ra")
