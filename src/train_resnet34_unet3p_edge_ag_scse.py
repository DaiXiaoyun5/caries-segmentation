...
def make_edge_from_mask(mask_array: np.ndarray, kernel_size: int = 3) -> np.ndarray:
    """
    mask_array: [H, W], value 0/1 or 0/255
    return edge: [H, W], value 0/1
    """
...
def estimate_edge_pos_weight(
    ...
    """
    Estimate positive edge pixel ratio on train set.
...
class AttentionGate(nn.Module):
    """
    Attention Gate for one feature branch.
    x: feature to be gated
    g: high-level gating feature
    """
...
# =========================
# ResNet34 Encoder + UNet3+ Decoder + Edge Branch + optional AG / scSE
# =========================
class ResNet34UNet3PlusEdgeAGSCSE(nn.Module):
    def __init__(
        self,
        ...
        # resnet34 usually: [3, 64, 64, 128, 256, 512]
        ch1, ch2, ch3, ch4, ch5 = (
            enc_channels[1],
            enc_channels[2],
            enc_channels[3],
        ...
        if self.use_ag:
            inter_channels = max(cat_channels // 2, 16)
            self.ag_hd4 = AttentionGate(cat_channels, cat_channels, inter_channels)
            self.ag_hd3 = AttentionGate(cat_channels, cat_channels, inter_channels)
            self.ag_hd2 = AttentionGate(cat_channels, cat_channels, inter_channels)
            self.ag_hd1 = AttentionGate(cat_channels, cat_channels, inter_channels)

        if self.use_scse:
            self.scse_hd4 = SCSEBlock(self.up_channels)
            self.scse_hd3 = SCSEBlock(self.up_channels)
            self.scse_hd2 = SCSEBlock(self.up_channels)
            self.scse_hd1 = SCSEBlock(self.up_channels)
    ...
    def apply_ag(self, ag_module, features, gating):
        if not self.use_ag:
            return features
        return [ag_module(feat, gating) for feat in features]
    ...
    def forward(self, x):
        ...
        # hd4
        h5_hd4 = self.up_to(self.h5_UT_hd4(h5), h4)
        ...
        if self.use_ag:
            hd4_parts = self.apply_ag(self.ag_hd4, hd4_parts, h5_hd4)

        hd4 = self.conv_hd4(torch.cat(hd4_parts + [h5_hd4], dim=1))
        if self.use_scse:
            hd4 = self.scse_hd4(hd4)

        # hd3
        h5_hd3 = self.up_to(self.h5_UT_hd3(h5), h3)
        h4_hd3 = self.up_to(self.h4_UT_hd3(hd4), h3)
        ...
        if self.use_ag:
            hd3_parts = self.apply_ag(self.ag_hd3, hd3_parts, h5_hd3)

        hd3 = self.conv_hd3(torch.cat(hd3_parts + [h5_hd3], dim=1))
        if self.use_scse:
            hd3 = self.scse_hd3(hd3)

        # hd2
        h5_hd2 = self.up_to(self.h5_UT_hd2(h5), h2)
        h3_hd2 = self.up_to(self.h3_UT_hd2(hd3), h2)
        h4_hd2 = self.up_to(self.h4_UT_hd2(hd4), h2)
        ...
        if self.use_ag:
            hd2_parts = self.apply_ag(self.ag_hd2, hd2_parts, h5_hd2)

        hd2 = self.conv_hd2(torch.cat(hd2_parts + [h5_hd2], dim=1))
        if self.use_scse:
            hd2 = self.scse_hd2(hd2)

        # hd1
        h5_hd1 = self.up_to(self.h5_UT_hd1(h5), h1)
        h2_hd1 = self.up_to(self.h2_UT_hd1(hd2), h1)
        h3_hd1 = self.up_to(self.h3_UT_hd1(hd3), h1)
        h4_hd1 = self.up_to(self.h4_UT_hd1(hd4), h1)
        ...
        if self.use_ag:
            hd1_parts = self.apply_ag(self.ag_hd1, hd1_parts, h5_hd1)

        hd1 = self.conv_hd1(torch.cat(hd1_parts + [h5_hd1], dim=1))
        if self.use_scse:
            hd1 = self.scse_hd1(hd1)
...
def main(args):
    ...
    model = ResNet34UNet3PlusEdgeAGSCSE(
    ...
    print("use_ag             =", bool(args.use_ag))
    print("use_scse           =", bool(args.use_scse))
    print("=" * 80)
    ...
    # Evaluate best checkpoint on train / val / test
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="smoke_resnet34_unet3p_edge_ag_scse")
    ...
    parser.add_argument("--use-scse", type=int, default=0)
