...
class ForegroundAwareCropCariesMaskDataset(torch.utils.data.Dataset):
    """
    Foreground-aware patch sampling dataset for CariXray segmentation.

    Training-only strategy:
    1. Read image and mask.
    2. Resize both to base_size x base_size, default 640.
    3. Crop a patch_size x patch_size patch, default 512.
    4. With probability fg_prob, force the crop to contain at least one foreground pixel
       according to the training mask.
    5. Otherwise, use a random crop.

    This uses mask only during training. Validation/test still use the original
    resize-based CariesMaskDataset, so evaluation does not use mask-guided cropping.
    """
    def __init__(self, split_file, base_size=640, patch_size=512, fg_prob=0.33):
        self.items = []
        self.base_size = int(base_size)
        self.patch_size = int(patch_size)
        self.fg_prob = float(fg_prob)

        if self.patch_size > self.base_size:
            raise ValueError(f"patch_size ({self.patch_size}) must be <= base_size ({self.base_size})")
    ...
    @staticmethod
    def _randint_inclusive(low, high):
        low = int(low)
        high = int(high)
        if high < low:
            return low
    ...
    def _sample_foreground_crop(self, mask_np):
        """
        Returns crop box left, top, right, bottom.
        The chosen crop contains at least one foreground pixel.
        """
        ys, xs = np.where(mask_np > 0)

        if len(xs) == 0:
            return self._sample_random_crop()

        idx = random.randint(0, len(xs) - 1)
        x = int(xs[idx])
        y = int(ys[idx])

        max_left = self.base_size - self.patch_size
        max_top = self.base_size - self.patch_size

        # Make sure selected foreground pixel is inside crop:
        # left <= x <= left + patch_size - 1
        left_min = max(0, x - self.patch_size + 1)
        left_max = min(x, max_left)

        top_min = max(0, y - self.patch_size + 1)
        top_max = min(y, max_top)

        left = self._randint_inclusive(left_min, left_max)
        top = self._randint_inclusive(top_min, top_max)

        return left, top, left + self.patch_size, top + self.patch_size

    def _sample_random_crop(self):
        max_left = self.base_size - self.patch_size
        max_top = self.base_size - self.patch_size
    ...
    def __getitem__(self, idx):
        ...
        # First enlarge/standardize to 640, then crop 512 patch.
        # This follows the idea of patch training while keeping final model input 512.
        image = image.resize((self.base_size, self.base_size), resample=Image.BILINEAR)
        mask = mask.resize((self.base_size, self.base_size), resample=Image.NEAREST)

        mask_np_for_sampling = np.asarray(mask)

        if random.random() < self.fg_prob and np.any(mask_np_for_sampling > 0):
            crop_box = self._sample_foreground_crop(mask_np_for_sampling)
        else:
            crop_box = self._sample_random_crop()
...
class ResNet34UNetCBSAMKDC(nn.Module):
    """
    P2:
    ResNet34-U-Net + CBSA + MKDC/MKIR refinement.

    Fairness design:
    - SMP ResNet34-U-Net main body unchanged.
    - CBSA placement follows previous M3/M4.
    - Official MK-UNet MKIR/MKDC block is extracted and inserted as a small
      decoder-output refinement block before segmentation head.
    """
    def __init__(
        self,
...
def main(args):
    ...
    config["experiment"] = "FG1/P2-foreground-aware crop: ResNet34-U-Net + CBSA + MKDC with mask-guided training patches"
    config["note"] = "Model architecture is unchanged from P2. Only training data sampling is changed: images/masks are resized to 640 and cropped to 512; 33 percent of training patches are forced to contain foreground caries pixels. Validation/test remain whole-image resize 512 without mask-guided cropping."
    save_json(config, run_dir / "config.json")

    train_ds = ForegroundAwareCropCariesMaskDataset(train_split, base_size=args.fg_base_size, patch_size=args.image_size, fg_prob=args.fg_prob)
    ...
    print("=" * 80)
    print("Model ready: P2 ResNet34-U-Net + CBSA + MKDC")
    print("run_name             =", args.run_name)
    print("train_sampling       = foreground-aware crop")
    print("fg_base_size         =", args.fg_base_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fg-base-size", type=int, default=640)
    parser.add_argument("--fg-prob", type=float, default=0.33)
