...
class CBSABlock(nn.Module):
    ...
    SimAM + feature-gradient boundary cue + residual scaling.
    """
...
class LiteASPPBlock(nn.Module):
    """
    Lite-ASPP for U-Net decoder output.

    SMP ResNet34-U-Net decoder output channel is usually 16.
    This Lite-ASPP keeps input/output channels unchanged, so it can be
    inserted before the original segmentation head.
    """
...
class ResNet34UNetCBSALiteASPP(nn.Module):
    """
    M4:
    ResNet34-U-Net + CBSA + Lite-ASPP.

    Pipeline:
    encoder -> decoder -> CBSA -> Lite-ASPP -> segmentation head.
    """
    def __init__(
        self,
        ...
        # SMP Unet default decoder output channel is segmentation_head[0].in_channels
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetCBSALiteASPP"
    ...
    model = ResNet34UNetCBSALiteASPP(
    ...
    print("=" * 80)
    print("Model ready: M4 ResNet34-U-Net + CBSA + Lite-ASPP")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_cbsa_laspp_e50_bs6")
