...
class BottleneckPPM(nn.Module):
    """
    Pyramid Pooling Module placed on the deepest encoder feature.

    For P2 with 512 input:
        f5: [B, 512, 16, 16]

    PPM aggregates multi-scale context by adaptive average pooling to
    1x1, 2x2 and 4x4, upsamples them back to 16x16, fuses them,
    then injects the context in a residual manner.
    """
    def __init__(self, in_channels=512, reduction_channels=128, pool_sizes=(1, 2, 4), beta=0.1):
        super().__init__()
        ...
        for ps in self.pool_sizes:
            self.branches.append(
                nn.Sequential(
                    nn.AdaptiveAvgPool2d(output_size=(ps, ps)),
        ...
        fusion_in_channels = in_channels + len(self.pool_sizes) * reduction_channels
...
class ResNet34UNetPPMCBSAMKDC(nn.Module):
    """
    ED0-PPM:
    ResNet34-U-Net + bottleneck PPM + CBSA + MKDC.

    Only modification against P2:
    - Insert PPM after the deepest ResNet34 encoder feature features[-1].
    - The PPM output is injected residually before the original SMP U-Net decoder.
    """
    def __init__(
        self,
        ...
        cbsa_alpha=1.0,
        cbsa_beta=0.1,
        ppm_reduction_channels=128,
        ppm_pool_sizes=(1, 2, 4),
        ppm_beta=0.1,
        ...
        self.bottleneck_ppm = BottleneckPPM(
            in_channels=512,
            reduction_channels=ppm_reduction_channels,
    ...
    def forward(self, x):
        ...
        features[-1] = self.bottleneck_ppm(features[-1])
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetPPMCBSAMKDC"
    config["experiment"] = "ED0-PPM: ResNet34-U-Net + bottleneck PPM + CBSA + MKDC"
    config["note"] = "Architecture is based on P2. A residual Pyramid Pooling Module is inserted after the deepest ResNet34 encoder feature to enhance multi-scale bottleneck context. Input processing, loss, CBSA and MKDC remain unchanged."
    ...
    model = ResNet34UNetPPMCBSAMKDC(
        ...
        ppm_reduction_channels=args.ppm_reduction_channels,
        ppm_pool_sizes=tuple(int(v) for v in args.ppm_pool_sizes.split(",")),
        ppm_beta=args.ppm_beta,
    ...
    print("=" * 80)
    print("Model ready: ED0-PPM: ResNet34-U-Net + bottleneck PPM + CBSA + MKDC")
    ...
    print("ppm_reduction_ch     =", args.ppm_reduction_channels)
    print("ppm_pool_sizes       =", args.ppm_pool_sizes)
    print("ppm_beta             =", args.ppm_beta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_cbsa_mkdc_ed0_ppm_e50_bs6")
    ...
    parser.add_argument("--ppm-reduction-channels", type=int, default=128)
    parser.add_argument("--ppm-pool-sizes", type=str, default="1,2,4")
    parser.add_argument("--ppm-beta", type=float, default=0.1)
