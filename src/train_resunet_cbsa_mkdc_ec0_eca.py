...
class ECABlock(nn.Module):
    """
    Efficient Channel Attention for encoder stage outputs.

    This implementation uses global average pooling followed by a lightweight
    1D convolution over channel descriptors. To avoid disrupting pretrained
    ResNet34 features too strongly, the attention is injected residually:

        out = x + beta * x * attention

    where attention is in [0, 1].
    """
    def __init__(self, kernel_size=3, beta=0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return x + self.beta * x * y


class ResNet34UNetEC0ECACBSAMKDC(nn.Module):
    """
    EC0:
    ResNet34-U-Net + high-level Encoder-ECA + final CBSA + MKDC.

    Compared with P2:
    - ResNet34 pretrained encoder structure is NOT modified internally.
    - ECA is inserted only after high-level encoder stage outputs:
        features[4]: layer3 output, [B, 256, 32, 32]
        features[5]: layer4 output, [B, 512, 16, 16]
    - features[1] and features[2] are left unchanged to avoid enhancing
      shallow dental/background texture noise.
    - Final decoder-output CBSA and MKDC are unchanged.
    """
    def __init__(
        self,
        ...
        # Encoder feature mapping for 512 input under SMP ResNet34:
        # features[4]: [B, 256, 32, 32], features[5]: [B, 512, 16, 16]
        self.eca_f4 = ECABlock(kernel_size=eca_kernel_size, beta=eca_beta)
        self.eca_f5 = ECABlock(kernel_size=eca_kernel_size, beta=eca_beta)
    ...
    def forward(self, x):
        ...
        # Only high-level encoder attention.
        features[4] = self.eca_f4(features[4])
        features[5] = self.eca_f5(features[5])
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetEC0ECACBSAMKDC"
    config["experiment"] = "EC0: ResNet34-U-Net + high-level Encoder-ECA + CBSA + MKDC"
    config["note"] = "Architecture is based on P2. ResNet34 internal BasicBlocks are unchanged. Residual ECA is inserted only after high-level encoder outputs features[4] and features[5]. Final CBSA and MKDC are unchanged."
    ...
    model = ResNet34UNetEC0ECACBSAMKDC(
    ...
    print("=" * 80)
    print("Model ready: EC0 ResNet34-U-Net + high-level Encoder-ECA + CBSA + MKDC")
    ...
    print("eca_kernel_size      =", args.eca_kernel_size)
    print("eca_beta             =", args.eca_beta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_cbsa_mkdc_ec0_eca_e50_bs6")
    ...
    parser.add_argument("--eca-kernel-size", type=int, default=3)
    parser.add_argument("--eca-beta", type=float, default=0.1)
