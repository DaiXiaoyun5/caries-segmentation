...
class OriginalScalePadCropCariesMaskDataset(torch.utils.data.Dataset):
    """
    Original-scale pad/crop dataset for CariXray segmentation.

    Difference from resize-based CariesMaskDataset:
    - Does NOT force-resize all images to target_size.
    - If an image is smaller than target_size, it is center-padded.
    - If an image is larger than target_size, it is center-cropped.
    - Aspect ratio and lesion pixel scale are preserved as much as possible.

    This is designed to test whether preserving original scale helps small caries lesions.
    """
    def __init__(self, split_file, target_size=(640, 640), image_fill=0, mask_fill=0):
        self.items = []
    ...
    @staticmethod
    def _center_crop_if_needed(image, mask, target_w, target_h):
        w, h = image.size
    ...
    def _center_pad_to_target(self, image, mask):
        target_w, target_h = self.target_size

        image, mask = self._center_crop_if_needed(image, mask, target_w, target_h)

        w, h = image.size
        if w == target_w and h == target_h:
            return image, mask

        canvas_img = Image.new("RGB", (target_w, target_h), color=(self.image_fill, self.image_fill, self.image_fill))
        canvas_mask = Image.new("L", (target_w, target_h), color=self.mask_fill)

        paste_left = (target_w - w) // 2
        paste_top = (target_h - h) // 2

        canvas_img.paste(image, (paste_left, paste_top))
        canvas_mask.paste(mask, (paste_left, paste_top))
    ...
    def __getitem__(self, idx):
        ...
        image, mask = self._center_pad_to_target(image, mask)
...
def main(args):
    ...
    config["experiment"] = "ORI1/P2-original-scale-pad640: ResNet34-U-Net + CBSA + MKDC with original-scale center pad/crop to 640"
    config["note"] = "Model architecture and loss are unchanged from P2. Input processing is changed from forced resize to original-scale center pad/crop at 640x640. No random augmentation is used."
    save_json(config, run_dir / "config.json")

    train_ds = OriginalScalePadCropCariesMaskDataset(train_split, target_size=(args.image_size, args.image_size))
    val_ds = OriginalScalePadCropCariesMaskDataset(val_split, target_size=(args.image_size, args.image_size))
    test_ds = OriginalScalePadCropCariesMaskDataset(test_split, target_size=(args.image_size, args.image_size))
    ...
    print("=" * 80)
    print("Model ready: ORI1/P2 original-scale pad/crop 640 + CBSA + MKDC")
    print("run_name             =", args.run_name)
    print("input_mode           = original-scale center pad/crop")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resunet_cbsa_mkdc_ori1_pad640_e80_bs4")
