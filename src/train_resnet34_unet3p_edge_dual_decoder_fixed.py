...
def make_edge_from_mask(mask_array: np.ndarray, kernel_size: int = 3) -> np.ndarray:
    ...
    edge = dilated - eroded
    return (edge > 0).astype(np.float32) # 【修复】强制 float32

def make_center_heatmap_from_mask(mask_array: np.ndarray, sigma=15) -> np.ndarray:
    """自动基于病灶 Mask 生成多中心高斯热力图"""
    heatmap = np.zeros_like(mask_array, dtype=np.float32) # 【修复】强制 float32
    mask_bin = (mask_array > 0).astype(np.uint8)
...
class CariesDualDataset(Dataset):
    def __init__(self, split_file, target_size=(512, 512), edge_kernel_size=3, heatmap_sigma=15, is_train=True):
        self.items =[]
        ...
        # 【修复】使用 Albumentations 替代旧的加载方式，防止过拟合
        if self.is_train:
    ...
    def __getitem__(self, idx):
        ...
        # 基于增强后的 Mask 生成边界和中心热力图
        edge_np = make_edge_from_mask(mask_tensor[0].numpy(), kernel_size=self.edge_kernel_size)
        center_np = make_center_heatmap_from_mask(mask_tensor[0].numpy(), sigma=self.heatmap_sigma)
        
        edge_tensor = torch.from_numpy(edge_np).unsqueeze(0).float()
        center_tensor = torch.from_numpy(center_np).unsqueeze(0).float()
        
        return image_tensor, mask_tensor, edge_tensor, center_tensor, str(img_path), str(mask_path)
...
def run_one_epoch(model, loader, optimizer, device, deep_supervision=True, lambda_edge=0.3, lambda_center=0.5, edge_pos_weight=1.0, train=True):
    ...
    # 【修复】强制 dtype 为 torch.float32，解决 Double 报错
    ...
    return {"loss": total_loss/total_batches, "seg_loss": total_seg_loss/total_batches, 
            ...
            "dice": total_dice/total_batches, "recall": total_recall/total_batches}
...
def main(args):
    ...
    train_loader = DataLoader(CariesDualDataset(project_root / "data/splits/train.txt", is_train=True, **ds_kwargs), batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    val_loader = DataLoader(CariesDualDataset(project_root / "data/splits/val.txt", is_train=False, **ds_kwargs), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="unet3p_edge_dual_decoder_fixed")
