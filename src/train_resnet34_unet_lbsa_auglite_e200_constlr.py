...
class LBSABlock(nn.Module):
    """
    Lesion-Boundary Saliency Attention (LBSA) block for final decoder features.

    Placement used in N2:
        image -> ResNet34 encoder -> SMP U-Net decoder -> LBSA -> segmentation head -> logits

    The block keeps the input/output shape unchanged, e.g. [B, 16, 512, 512].
    It combines parameter-free SimAM-style spatial saliency with a learned-feature
    gradient boundary cue, then injects the attended feature by a small residual scale.
    """
    def __init__(self, e_lambda=1e-4, alpha=1.0, beta=0.1):
        super().__init__()
        self.e_lambda = e_lambda
    ...
    def feature_boundary_cue(self, x):
        # Feature-level first-order gradient. Compared with fixed image gradients,
        # this cue is computed on decoder features and is therefore more task-adaptive.
        gx = torch.abs(x[:, :, :, 1:] - x[:, :, :, :-1])
        gx = F.pad(gx, (0, 1, 0, 0))
        gy = torch.abs(x[:, :, 1:, :] - x[:, :, :-1, :])
        gy = F.pad(gy, (0, 0, 0, 1))
    ...
    def forward(self, x):
        saliency = self.simam_attention(x)
        boundary = self.feature_boundary_cue(x)
        attention = saliency * (1.0 + self.alpha * boundary)
        return x + self.beta * x * attention


class ResNet34UNetLBSA(nn.Module):
    """
    N2: Plain ResNet34-U-Net + LBSA with AugLite training.

    This model keeps the strongest plain ResNet34-U-Net baseline unchanged and inserts
    a lightweight Lesion-Boundary Saliency Attention block between the SMP U-Net
    decoder and the segmentation head.

    Forward:
        image -> ResNet34 encoder -> SMP U-Net decoder -> LBSA -> segmentation head -> mask logits
    """
    def __init__(
        self,
        ...
        lbsa_alpha=1.0,
        lbsa_beta=0.1,
        # Keep legacy/ignored arguments for compatibility with config/eval scripts.
        cbsa_lambda=1e-4,
        cbsa_alpha=1.0,
        cbsa_beta=0.1,
        ...
        self.lbsa = LBSABlock(e_lambda=lbsa_lambda, alpha=lbsa_alpha, beta=lbsa_beta)

    def forward(self, x):
        features = self.unet.encoder(x)
        decoder_output = self.unet.decoder(*features)
        decoder_output = self.lbsa(decoder_output)
...
def main(args):
    ...
    config["model_class"] = "ResNet34UNetLBSA"
    config["experiment"] = "N2-LBSA: ResNet34-U-Net + Lesion-Boundary Saliency Attention with AugLite, 200 epochs, constant lr"
    config["note"] = "N2 adds a Lesion-Boundary Saliency Attention block after the SMP U-Net decoder and before the segmentation head. Other training settings are kept identical to the plain AugLite E200 ConstLR baseline."
    ...
    model = ResNet34UNetLBSA(
        encoder_name=args.encoder_name,
        encoder_weights=args.encoder_weights,
        in_channels=3,
        num_classes=1,
        lbsa_lambda=args.lbsa_lambda,
        lbsa_alpha=args.lbsa_alpha,
        lbsa_beta=args.lbsa_beta,
    ...
    print("=" * 80)
    print("Model ready: N2 ResNet34-U-Net + LBSA + AugLite, without MKDC")
    print("run_name             =", args.run_name)
    print("encoder_name         =", args.encoder_name)
    print("encoder_weights      =", args.encoder_weights)
    print("lbsa_lambda          =", args.lbsa_lambda)
    print("lbsa_alpha           =", args.lbsa_alpha)
    print("lbsa_beta            =", args.lbsa_beta)
    print("cbsa_lambda_legacy   =", args.cbsa_lambda)
    print("cbsa_alpha_legacy    =", args.cbsa_alpha)
    print("cbsa_beta_legacy     =", args.cbsa_beta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="resnet34_unet_lbsa_auglite_e200_constlr_bs6")
    ...
    parser.add_argument("--lbsa-lambda", type=float, default=1e-4)
    parser.add_argument("--lbsa-alpha", type=float, default=1.0)
    parser.add_argument("--lbsa-beta", type=float, default=0.1)

    # Legacy names kept only so config/eval scripts that expect CBSA-style args remain harmless.
